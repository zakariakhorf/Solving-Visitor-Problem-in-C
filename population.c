...
void selectionMeilleursEtClonesMutesMethod2(Population *population)
{
  ...
  Ac *tableau[population->nbIndividus + population->nbClones];
  int count = 0, count2 = 0;
  for (count = population->nbIndividus - population->nbClones; count < population->nbIndividus; count++, count2++)
  {
    tableau[count2] = population->individus[count];
  }

  for (count = 0; count < population->nbClones; count++, count2++)
  {
    tableau[count2] = population->clones[count];
  }

  int i, j;
  Ac *c;
  for (i = 0; i < (population->nbClones) * 2; i++)
  {
    for (j = i + 1; j < population->nbClones + 1; j++)
    {
      if (tableau[i].cout < tableau[j].cout)
      {
        c = tableau[i];
        tableau[i] = tableau[j];
        tableau[j] = c;
  ...
  for (count = population->nbIndividus - population->nbClones; count < population->nbIndividus; count++, count2++)
  {
    population->individus[count] = tableau[count2];
  }
  for (count = 0; count < population->nbClones; count++, count2++)
  {
    population->clones[count] = tableau[count2];
  }
}
